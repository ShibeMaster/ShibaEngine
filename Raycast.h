...
class Raycast {
public:
	static bool IntersectsCollider(const Ray& ray, const MeshCollisionBox& collider, float* distance = nullptr, int entity = -1) {
	...
	static bool CheckCollision(Ray& ray, std::vector<RayHit>* outHit, int excludeEntity = -1) {
	...
	static bool CheckHit(glm::vec3 origin, glm::vec3 direction, float distance, std::vector<RayHit>* outHits = nullptr, int excludeEntity = -1) {
