...
class GameView {
public:
	View view = View(glm::vec2(Display::width * 0.3f, Display::height * 0.25f), glm::vec2(Display::width * 0.45f, Display::height * 0.75f));

	void Update(bool inRuntime) {
		if (!view.hasCamera) {
			auto cams = Engine::FindComponentsInScene<Camera>();
			if(cams.size() > 0)
				view = View(glm::vec2(Display::width * 0.3f, Display::height * 0.25f), glm::vec2(Display::width * 0.45f, Display::height * 0.75f), Engine::GetComponentPointer<Camera>(cams[0]));
