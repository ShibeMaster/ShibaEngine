...
unsigned int ModelLoader::LoadCubeMap(std::vector<std::string> sides) {
	unsigned int id;
	glGenTextures(1, &id);
	...
	for (int i = 0; i < sides.size(); i++) {
		unsigned char* data = stbi_load(sides[i].c_str(), &width, &height, &channels, 0);
		if (data) {
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
			stbi_image_free(data);
		}
		else {
			std::cout << "failed to load cubemap texture: " << i << " at path " << sides[i].c_str() << std::endl;
			stbi_image_free(data);
		}
	}
...
Sprite ModelLoader::LoadSprite(const std::string& path) {
	Sprite sprite;
...
void ModelLoader::ProcessNode(Model* model, aiNode* node, const aiScene* scene, const std::string& directory) {
...
void ModelLoader::LoadMesh(aiMesh* mesh, Model* model, const aiScene* scene, const std::string& directory) {
	std::vector<Vertex> vertices;
...
int ModelLoader::GenerateTexture(const char* path, const std::string& directory) {
